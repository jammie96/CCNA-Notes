...
<insert picture here>
- An application on host A wishes to snd data to a remote network
...
- When the switch receives the frame, it needs to forward it out on the proper port
 ...
 - The switch can learn the port mapping for the source host from the source MAC address in the frame, so the switch adds the informaiton to the table (0800:0222:2222 = port FastEthernet0/1)
